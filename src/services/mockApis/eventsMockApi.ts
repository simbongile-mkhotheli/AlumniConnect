 ...
 */
const filterEvents = (events: Event[], filters: FilterState): Event[] => {
...
export class EventsMockApiService {
  ...
  static async rsvpToEvent(
    ...
    try {
      ...
      // Simulate checking if user already RSVPed (in real app, this would check database)
      const existingRsvps = JSON.parse(
        localStorage.getItem('mockRsvps') || '{}'
      );
      ...
      console.log(
        `✅ RSVP successful for user ${rsvpData.userId} to event ${id}`
      );
  ...
  static async cancelRsvp(
    ...
    try {
      ...
      const userRsvps = existingRsvps[userId] || [];

      if (!userRsvps.includes(id)) {
        return {
          data: { success: false, message: 'No RSVP found for this event' },
          success: false,
          message: 'No RSVP found',
          error: {
            code: 400,
            message: 'User has not RSVPed to this event',
          },
        };
      }
      ...
      existingRsvps[userId] = updatedUserRsvps;
      localStorage.setItem('mockRsvps', JSON.stringify(existingRsvps));
      ...
      console.log(`❌ RSVP cancelled for user ${userId} to event ${id}`);

      return {
        data: { success: true, message: 'RSVP cancelled successfully' },
        success: true,
        message: 'RSVP cancelled successfully',
      };
    } catch (error) {
      return {
        data: { success: false, message: 'Failed to cancel RSVP' },
        success: false,
      ...
      const mockAttendees = Array.from(
        ...
        (_, index) => ({
          ...
          attendanceStatus:
            Math.random() > 0.3
              ? 'registered'
              : ('attended' as 'registered' | 'attended' | 'no_show'),
          ticketType: 'general',
        })
      );
  ...
  static async searchEvents(
    ...
    try {
      ...
      if (filters) {
        filteredResults = filterEvents(searchResults, filters as FilterState);
      }
  ...
  static async getEventsBySponsor(
    ...
    try {
      ...
      const sponsorEvents = events.filter(
        event => event.sponsor?.toLowerCase() === sponsor.toLowerCase()
      );
      ...
      const stats = {
        ...
        averageAttendanceRate:
          events.length > 0
            ? Math.round(
                (events.reduce((sum, e) => {
                  const rate = e.rsvpCount
                    ? (e.attendanceCount || 0) / e.rsvpCount
                    : 0;
                  return sum + rate;
                }, 0) /
                  events.length) *
        ...
        pastEvents: events.filter(
          e =>
            e.status === 'published' && new Date(e.endDate || e.startDate) < now
        ).length,
