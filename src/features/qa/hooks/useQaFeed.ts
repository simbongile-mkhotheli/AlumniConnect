...
export function useQaFeed(options: UseQaFeedOptions = {}): UseQaFeedResult {
  ...
  // Instantiate feature QA service (feature service methods are instance-based)
  const qaService = useMemo(() => new QAService(), []);

  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await qaService.getQuestions({ ...(filters as any), page, limit: pageSize });
      const dataArr: QAItem[] = (response.data as any[]) || [];
      const totalCount: number = (response as any)?.pagination?.total ?? dataArr.length;
      setTotal(totalCount);
  ...
  }, [page, pageSize, filters, limitQuestions, qaService]);
  ...
  const setFilters = (f: Partial<FilterState>) => {
    setPage(1);
    setFiltersState((prev: Partial<FilterState>) => ({ ...prev, ...f }));
  };
