import type { User, Event, Sponsor, Partner, Chapter, Opportunity, Mentorship, QAItem, Spotlight, DbUser } from '@shared/types';
...
export class MockDataLoader {
  ...
  private static ensureEnabled(op: string) {
    if (!this.isEnabled()) {
      ...
      if (!isRead && !this.warned) {
        console.warn(`MockDataLoader disabled (VITE_ENABLE_MOCK_API!=="true"). '${op}' will no-op.`);
        this.warned = true;
      }
      return false;
    }
